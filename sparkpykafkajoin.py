...
spark = SparkSession.builder.appName("join-record").getOrCreate()
...
emailAndBirthDayStreamingDF = spark.sql("select email as email, birthDay as birthDay from CustomerRecords WHERE email is NOT NULL AND birthDay IS NOT NULL")

# TO-DO: Split the birth year as a separate field from the birthday
# TO-DO: Select only the birth year and email fields as a new streaming data frame called 
emailAndBirthYearStreamingDF = emailAndBirthDayStreamingDF.select('email', 'birthDay',split(emailAndBirthDayStreamingDF.birthDay,"-").getItem(0).alias("birthYear"))

# TO-DO: using the spark application object, read a streaming dataframe from the Kafka topic stedi-events as the source
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
spark = SparkSession.builder.appName("events-record").getOrCreate()
spark.sparkContext.setLogLevel('WARN')
...
StediEventsStreamingDF.withColumn("value",from_json("value",customerRiskSchema))\
        .select(col('value.*')) \
        .createOrReplaceTempView("CustomerRisk")

# TO-DO: execute a sql statement against a temporary view, selecting the customer and the score from the temporary view, creating a dataframe called customerRiskStreamingDF
customerRiskStreamingDF = spark.sql("select customer as customer, score as score from CustomerRisk")

# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same dataframe
customerRiskAndBirthYearDF = customerRiskStreamingDF \
